...
class Particles {
  ...
  initParticles() {
    ...
    for (let i = 0; i < 5; i++) {
      const cluster = {
        scale: i + 2,
        speed: THREE.Math.randFloat(2.8, 3.8),
  ...
  getCluster(count) {
    ...
    for (let i = 0; i < count; i++) {
      ...
      p.x = THREE.Math.randFloatSpread(1);
      p.y = THREE.Math.randFloatSpread(1);
      p.z = THREE.Math.randFloatSpread(1);
      geo.vertices.push(p);
    }
  ...
  update(delta) {
    for (let i = 0; i < this.clusters.length; i++) {
      ...
      cluster.scale += 0.65 * delta * cluster.speed;
...
class App {
  ...
  beforeInit(){
    ...
    let opacity = getComputedStyle(middle).getPropertyValue('opacity');      
        opacity = getComputedStyle(middle).getPropertyValue('opacity');
        middle.style.opacity = opacity - 1.;                             
